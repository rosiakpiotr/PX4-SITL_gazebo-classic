                            ...
                            sdf::ElementPtr _sdf) // NOLINT
{
    ...
    command_vpp_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandPitchAngle>(
    ...
    vpp_state_pub_ = node_handle_->Advertise<mav_msgs::msgs::VppState>(
...
void GazeboMotorModel::PitchAngleCallback(CommandPitchAnglePtr &pitch_angles) {
    ...
    if (pitch_angles->pitch_angle_size() < motor_number_) {
        ...
        // std::cout << "Pitch angle for motor " << motor_number_ << ": " <<
        // pitch_angle << '\n';
