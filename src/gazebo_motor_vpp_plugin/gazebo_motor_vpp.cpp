...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) // NOLINT
{
    model_ = _model;
    ...
    command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_,
                   &GazeboMotorModel::VelocityCallback, this);
    command_vpp_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandPitchAngle>("~/" + model_->GetName() +
    ...
    vpp_state_pub_ = node_handle_->Advertise<mav_msgs::msgs::VppState>("~/" + model_->GetName() + vpp_state_pub_topic_, 10);
...
static double last_sent_timestamp = 0; // NOLINT
static bool send_diagnostics_now = true; // NOLINT
...
void GazeboMotorModel::OnUpdate(const common::UpdateInfo &_info)
{
    ...
    if ((_info.simTime.Double() - last_sent_timestamp) > 0.01) {
...
void GazeboMotorModel::UpdateForcesAndMoments()
{
    ...
    ignition::math::Vector3d velocity_perpendicular_to_rotor_axis = relative_wind_velocity - (relative_wind_velocity.Dot(
                joint_axis)) *
            joint_axis;
...
#else
    ignition::math::Pose3d pose_difference = ignitionFromGazeboMath(link_->GetWorldCoGPose() - parent_links.at(
                0)
            ->GetWorldCoGPose());
#endif
    ...
    if (send_diagnostics_now && (motor_number_ == 0)) {
        ...
        // if (dur > 5) {
        //     std::cout << "UpdateForcesAndMoments() took: " << dur << " microseconds" << '\n';
        // }
    }
}
...
void GazeboMotorModel::UpdateMotorVelocity()
{
    ...
    if (use_pid_) {
        ...
        double err = -((ref_motor_rot_vel / rotor_velocity_slowdown_sim_) - measVelocity);
